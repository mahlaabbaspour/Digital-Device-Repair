...
import { useUpdateCourseStatusDetermination } from '@/hooks/organization/useLessonEducation'

export default function CourseStatusDeterminationModal({
  onClose,
  open,
  id,
  rowSelect
  ...
  const { mutateAsync, isPending }: any = useUpdateCourseStatusDetermination()

  async function onSubmit(values: any) {
    try {
      await toast.promise(mutateAsync({ data: values, id, rowId: rowSelect?.id }), {
          ...
          <DialogContent sx={{ pb: 6, px: { xs: 8, sm: 15 }, pt: { xs: 8, sm: 12.5 }, position: 'relative' }}>
            ...
            <CardContent>
              <Grid container spacing={5}>
                            ...
                            label={
                              <Box
                                sx={{
                                  ...
                                  cursor: 'pointer',
                                  fontWeight: 700,
                                  color: field.value == 3 ? '#fff' : '#d0d17aff',
                                  background:
                                    field.value == 3 ? 'linear-gradient(135deg,#d0d17aff,#d0d17aff)' : '#f5f3e8ff',
                                  boxShadow: field.value == 3 ? '0 8px 24px rgba(218, 228, 81, 0.4)' : 'none'
                                }}
                              >
                                عودت برای اصلاح
                              </Box>
                ...
                {(watch('course_status_id') == 3 || watch('course_status_id') == 5) && (
                  <Grid item xs={12}>
                    <Controller
                      name='objection'
                      control={control}
                      render={({ field: { onChange, value }, fieldState: { error } }) => (
                        <Autocomplete
                          ...
                          renderInput={params => (
                            <TextField error={!!error} helperText={error?.message} {...params} label='دلایل' />
                          )}
                        />
                      )}
                    />
