...
import { useUpdateHistoryEducationRevisionTeachingPermission } from '@/hooks/organization/useRevisionTeachingPermission'

export default function UpdateHistoryEducationRevisionModal({
  onClose,
  open,
  id,
  revisionId,
  ...
  useEffect(() => {
    if (rowSelect) {
      ...
      setValue('region_id', rowSelect?.region)
      setValue('objection', rowSelect?.objection ? rowSelect?.objection : [])
      setValue('status', rowSelect?.status ? rowSelect?.status : '0')
    }
  }, [rowSelect])

  const { mutateAsync, isPending }: any = useUpdateHistoryEducationRevisionTeachingPermission()
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogContent sx={{ pb: 6, px: { xs: 8, sm: 15 }, pt: { xs: 8, sm: 12.5 }, position: 'relative' }}>
            ...
            <CardHeader
              ...
              subheader={<Typography variant='caption'>می توانید اطلاعات سوابق آموزشی خود را وارد کنید</Typography>}
            />
            <Divider />
            <CardContent>
