...
import { useCreateConsultationSubsidyCriteria } from '@/hooks/admin/base/useConsultationSubsidyCriteria'
import { useCreateEducation } from '@/hooks/admin/base/useEducation'
...
export default function ModalCreateConsultationSubsidyCriteria({
  onClose,
  open,
  title,
  description
  ...
  const { mutateAsync, isPending }: any = useCreateConsultationSubsidyCriteria()
    ...
    <Dialog TransitionComponent={Transition} fullWidth open={open} maxWidth='md' scroll='body'>
      <form onSubmit={handleSubmit(onSubmit)}>
        <DialogContent sx={{ pb: 6, px: { xs: 8, sm: 15 }, pt: { xs: 8, sm: 12.5 }, position: 'relative' }}>
          ...
          <Grid container spacing={5} mb={10} mt={2}>
            <Grid item xs={9}>
              <Controller
                name='name'
                control={control}
