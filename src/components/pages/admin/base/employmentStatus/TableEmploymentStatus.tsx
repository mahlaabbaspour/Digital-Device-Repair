...
export default function EmploymentStatusTable() {
    ...
    return (
        <>
        <ModalCreateEmploymentStatus  title="ایجاد وضعیت اشتغال" description="می توانید وضعیت اشتغال مورد نظر را ایجاد کنید" open={modalOpenCreate} onClose={() => setModalOpenCreate(false)} />
        <ModalUpdateEmploymentStatus action={action}  currentRow={currentRow} title={`${action === 'edit' ? 'ویرایش وضعیت اشتغال' : 'نمایش  وضعیت اشتغال'}`} description={`${action === 'edit' ? 'می توانید وضعیت اشتغال مورد نظر را ویرایش کنید' : 'می توانید وضعیت اشتغال مورد نظر را مشاهده کنید'}`} open={modalOpen} onClose={() => setModalOpen(false)} />   
        <CustomTable
            ...
            cardHeader={{
                status: true,
                btn: <Button variant='contained' color="primary" onClick={() => openCreateModal()}>ایجاد وضعیت اشتغال جدید</Button>
            }}
        />
        </>
    )
