...
import { useGetCustomer, useUpdateCustomer } from '@/hooks/admin/customer/useCustomer'
...
export default function CustomerEdit({ open, id, onClose }: Props) {
  const { mutateAsync: updateCustomer, isPending } = useUpdateCustomer()
  const { data: expertData } = useGetCustomer(id)
  ...
  useEffect(() => {
    if (expertData) {
      reset({
        ...
        address: expertData.data.address,
    ...
    >
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box>
            <DialogTitle sx={{ p: 0, fontSize: '1.2rem', fontWeight: 700 }}>ویرایش مشتری</DialogTitle>
