...
import { useGetOperator, useUpdateOperator } from '@/hooks/admin/operator/useOperator'
...
export default function OperatorEdit({ open, id, onClose }: Props) {
  const { mutateAsync: updateOperator, isPending } = useUpdateOperator()
  const { data: operatorData } = useGetOperator(id)
  ...
  useEffect(() => {
    if (operatorData) {
      reset({
        first_name: operatorData.data.first_name,
        last_name: operatorData.data.last_name,
        national_code: operatorData.data.national_code,
        mobile: operatorData.data.mobile,
        status: operatorData.data.status
      })
    }
  }, [operatorData, reset])
    ...
    >
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box>
            <DialogTitle sx={{ p: 0, fontSize: '1.2rem', fontWeight: 700 }}>ویرایش اپراتور</DialogTitle>
            ...
            <Controller
              ...
              render={({ field, fieldState }) => (
                <TextField
                  {...field}
                  label='نام خانوادگی'
                  placeholder='نام خانوادگی اپراتور را وارد کنید'
                  fullWidth
                  error={!!fieldState.error}
                  helperText={fieldState.error?.message}
                />
