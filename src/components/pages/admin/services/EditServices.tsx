...
import { useGetServices, useUpdateServices } from '@/hooks/admin/services/useServices'
...
export default function ServicesEdit({ open, id, onClose }: Props) {
  const { mutateAsync: updateService, isPending } = useUpdateServices()
  const { data: servicesData } = useGetServices(id)
  ...
  useEffect(() => {
    if (servicesData) {
      reset({
        ...
        sale_price: servicesData.data.sale_price,
        status: servicesData.data.status
      })
    }
  }, [servicesData, reset])
    ...
    >
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box>
            <DialogTitle sx={{ p: 0, fontSize: '1.2rem', fontWeight: 700 }}>ویرایش خدمت</DialogTitle>
