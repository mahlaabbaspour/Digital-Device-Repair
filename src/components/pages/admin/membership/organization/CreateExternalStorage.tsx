...
import { useCreateExternalStorage, useUpdateExternalStorage } from '@/hooks/admin/membership/useExternalServer'
...
function CreateExternalStorage({ action, currentRow, id, open, title, description, setOpen, disabled }: any) {
  const {
    handleSubmit,
    control,
    formState: { errors },
  ...
  const { mutateAsync: create, isPending: isLoadingCreate }: any = useCreateExternalStorage()
  const { mutateAsync: update, isPending: isLoadingUpdate }: any = useUpdateExternalStorage()
