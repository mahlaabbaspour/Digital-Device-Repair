...
import { useGetExpert, useUpdateExpert } from '@/hooks/admin/expert/useExpert'
...
export default function ExpertEdit({ open, id, onClose }: Props) {
  const { mutateAsync: updateExpert, isPending } = useUpdateExpert()
  const { data: expertData } = useGetExpert(id)
  ...
  useEffect(() => {
    if (expertData) {
      reset({
        first_name: expertData.data.first_name,
        last_name: expertData.data.last_name,
        national_code: expertData.data.national_code,
        mobile: expertData.data.mobile,
        status: expertData.data.status
      })
    }
  }, [expertData, reset])
    ...
    >
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box>
            <DialogTitle sx={{ p: 0, fontSize: '1.2rem', fontWeight: 700 }}>ویرایش کارشناس</DialogTitle>

            <Typography variant='body2' color='text.secondary' sx={{ mt: 0.5 }}>
              اطلاعات کارشناس را ویرایش کنید
            </Typography>
          </Box>
        </Box>
      </Box>
