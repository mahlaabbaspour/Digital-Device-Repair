...
import CustomRadioIcons from '@/components/elements/CustomRadioIcons'
import {
  Box,
  Card,
  CardContent,
...
import { BiFemale, BiFile, BiMoney, BiUser } from 'react-icons/bi'
import { useRouter } from 'next/navigation'
import moment from 'moment-jalaali'
import { useUpdateDocumentAndDiagnoisis } from '@/hooks/superUser/useDiagnosis'
...
export default function DiagnosisTable({ id, documentId, show }: any) {
  ...
  const handleRadioChange = (prop: number | ChangeEvent<HTMLInputElement>) => {
    if (typeof prop === 'number') {
      setSelectedRadio(prop)
    } else {
      setSelectedRadio(Number(prop.target.value))
    }
  }

  // منطق اسکرول کارت جلسات
  ...
  useEffect(() => {
    if (!show?.consultation_meetings) return

    const mappedForms: Record<number, any> = {}

    show.consultation_meetings.forEach((meeting: any) => {
  ...
  const { mutateAsync: updateDocument, isPending: isLoadingDocument } = useUpdateDocumentAndDiagnoisis()
        ...
        >
          <Card sx={{ mt: 10, position: 'relative', width: '84rem' }}>
            <CardContent sx={{ position: 'relative' }}>
              ...
              >
                {show?.consultation_meetings?.map((item: any) => (
                  <Box
                    key={item.id}
                    sx={{
