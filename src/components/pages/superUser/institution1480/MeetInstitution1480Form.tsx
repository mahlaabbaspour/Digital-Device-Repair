...
import { useCreateMeetingAndDiagnosisInstitution } from '@/hooks/superUser/useInstitution1480'
import { GeTErrorFetch } from '@/components/elements/errorHandler'

export default function CreateMeetInstitution1480Form({ id, upsertData }: any) {
  const [userData, setUserData] = useState<any>(null)
  const fetchAdvisorInfo = async (userId: number) => {
    try {
      const res = await axiosConfig.get(`/super-user/${id}/meeting-1480/core/meeting-1480/get-user`, {
        params: {
          user_id: userId
        }
      })
  ...
  useEffect(() => {
    ...
    if (!user) {
      ...
      setValue('username', '')
      setValue('mobile', '')
      setValue('gender_id', null)
      setValue('nationality_id', null)
      setValue('education_id', null)
      setValue('marital_id', null)
      setValue('employment_status_id', null)
    }
  }, [userData, user])
  ...
  const { mutateAsync, isPending }: any = useCreateMeetingAndDiagnosisInstitution()
  ...
  async function onSubmit(values: any) {
    try {
      ...
      if (res?.status) {
        router.push(`/superUser/${id}/institution1480/meetings`)
      }
    } catch (error) {
            ...
            <Grid container spacing={5}>
              ...
              <Grid item xs={12} md={3}>
                <Controller
                  ...
                  render={({ field: { onChange, value }, fieldState: { error } }) => (
                    <CustomAsyncAutocomplete
                      url={`/super-user/${id}/meeting-1480/base/select/marital`}
                      readOnly={false}
                      onAddValue={newValue => onChange(newValue)}
                      value={value}
                      getOptionLabel={optien => optien?.name}
              ...
              <Grid item xs={12} md={3}>
                <Controller
                  ...
                  render={({ field: { onChange, value }, fieldState: { error } }) => (
                    <CustomAsyncAutocomplete
                      url={`/super-user/${id}/meeting-1480/base/select/nationality`}
                      readOnly={false}
                      onAddValue={newValue => onChange(newValue)}
                      value={value}
                      getOptionLabel={optien => optien?.name}
              ...
              <Grid item xs={12} md={3}>
                <Controller
                  ...
                  render={({ field: { onChange, value }, fieldState: { error } }) => (
                    <CustomAsyncAutocomplete
                      url={`/super-user/${id}/meeting-1480/base/select/education`}
                      readOnly={false}
                      onAddValue={newValue => onChange(newValue)}
                      value={value}
                      getOptionLabel={optien => optien?.name}
              ...
              <Grid item xs={12} md={3}>
                <Controller
                  ...
                  render={({ field: { onChange, value }, fieldState: { error } }) => (
                    <CustomAsyncAutocomplete
                      url={`/super-user/${id}/meeting-1480/base/select/employment-status`}
                      readOnly={false}
                      onAddValue={newValue => onChange(newValue)}
                      value={value}
                      getOptionLabel={optien => optien?.name}
              ...
              <Grid item xs={12} md={3}>
                <Controller
                  ...
                  render={({ field: { onChange, value }, fieldState: { error } }) => (
                    <CustomAsyncAutocomplete
                      url={`/super-user/${id}/meeting-1480/base/select/service-recipient-type`}
                      readOnly={false}
                      onAddValue={newValue => onChange(newValue)}
                      value={value}
                      getOptionLabel={optien => optien?.name}
