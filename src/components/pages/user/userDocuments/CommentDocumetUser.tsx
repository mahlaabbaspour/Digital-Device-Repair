...
import {
  ...
  useDeleteCommentUser,
  useFetchComment,
  useUpdateCommentUser
} from '@/hooks/user/useUserDocument'
...
export default function CommentUser({ documentId, id, advisorId }: any) {
  ...
  const { data, isLoading }: any = useFetchComment({
    id: id,
    rowId: documentId,
    advisorId: advisorId
  })
  ...
  const { mutateAsync, isPending }: any = useCreateCommentUser()
  const { mutateAsync: update, isPending: loadingUpdate }: any = useUpdateCommentUser()
  const { mutateAsync: deleteComment, isPending: loadingDelete }: any = useDeleteCommentUser()
  ...
  const handleUpdateMessage = async () => {
    try {
      ...
      const res: any = await toast.promise(
        update({ data: data, id: id, rowId: documentId, commentId: selectMessgeId, advisorId: advisorId }),
        {
          pending: 'در حال ارسال..'
        }
      )
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', maxWidth: 'calc(100% - 40px)' }}>
                    ...
                    >
                      ...
                      <Typography
                        ...
                        sx={{
                          display: 'block',
                          textAlign: message?.sender === 'userOnline' ? 'left' : 'right',
