...
export default function DialogAlertDeleteReserverMeeting({
  title,
  description,
  open,
  onClose,
  ...
  const handleRequestDelete = async () => {
    try {
      const res: any = await toast.promise(deleteFun({ id: id, meetingId: selectedRow?.id, params }), {
        pending: 'در حال انجام...'
      })
      onClose()

      if (res?.status) {
        toast.success(res?.data ? res?.data[0] : 'با موفقیت انجام شد')
        onClose()
      } else {
