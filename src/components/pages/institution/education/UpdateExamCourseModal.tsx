...
import { useUpdateExamCourseInstitution } from '@/hooks/institution/education/useLessonEducation'
...
export default function UpdateExamModal({
  ...
  useEffect(() => {
    if (rowSelect) {
      setValue('exam_duration', rowSelect?.exam_duration)
      setValue('passing_score', rowSelect?.passing_score)
      setValue('total_question', rowSelect?.total_question)
      setValue('max_attempt', rowSelect?.max_attempt)
      setValue('exam_interval', rowSelect?.exam_interval)
      setValue('exam_nature_id', rowSelect?.examNature)
      setValue('exam_delivery_method_id', rowSelect?.examDeliveryMethod)
      setValue('exam_type_id', rowSelect?.examType)
      setValue(
        'start_time',
        //@ts-ignore
        rowSelect?.start_time ? parse(rowSelect?.start_time, 'yyyy/MM/dd HH:mm', new Date()) : null
      )
      setValue(
        'end_time',
        //@ts-ignore
        rowSelect?.end_time ? parse(rowSelect?.end_time, 'yyyy/MM/dd HH:mm', new Date()) : null
      )
      setInstruction(rowSelect?.instruction ? htmlToEditorState(rowSelect?.instruction) : EditorState.createEmpty())
    }
  }, [rowSelect])

  const { mutateAsync, isPending }: any = useUpdateExamCourseInstitution()
            ...
            <CardHeader
              ...
              title={
                <Typography variant='h5' sx={{ fontWeight: 'bold' }}>
                  {disabled ? 'نمایش آزمون' : 'ویرایش آزمون'}
                </Typography>
              }
              subheader={
                <Typography variant='caption'>
                  {disabled ? 'می توانید آزمون مورد نظر را مشاهده کنید' : 'می توانید آزمون مورد نظر را ویرایش کنید'}
                </Typography>
              }
            />
            <CardContent>
