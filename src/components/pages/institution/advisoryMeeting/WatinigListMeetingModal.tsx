...
export default function CreateWatingListMeeting({
  ...
  const { data, isLoading: loadingData } = useFetchShowWatingList({
    date: date,
    id: id,
  ...
  const { mutateAsync: waitingList, isPending: isLoading }: any = useCreateWatingList()
  ...
  const handleRequest = async (data: any) => {
    try {
      ...
      const res: any = await toast.promise(
        waitingList({ data: dataWating, id: id, rowId: selectedRow?.id, date: date }),
        {
          pending: 'در حال انجام...'
        }
      )
  ...
  const { mutateAsync: deletewaiting, isLoading: loadWating }: any = useDeleteWatingList()

  const handleDeleteWaiting = async (watiId: string) => {
    try {
      await toast.promise(deletewaiting({ id: id, rowId: selectedRow?.id, waitId: watiId }), {
        pending: 'در حال انجام...'
      })
    } catch (error) {
      throw error
