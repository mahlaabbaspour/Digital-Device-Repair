...
import {
  useCreateCheckSingleClanderConsultation,
  useCreateSingleMeetingCalenderConsultation
...
export default function ModalSingleMeeting({
  ...
  const { mutateAsync, isPending }: any = useCreateCheckSingleClanderConsultation()
  const { mutateAsync: create, isPending: loading }: any = useCreateSingleMeetingCalenderConsultation()
  ...
  async function onSubmit(values: any) {
    try {
      ...
      if (!response?.status) {
        toast.error('جلسه مورد نظر تداخل زمانی با دیگر جلسات مشاور دارد.')
      }

      if (response?.status) {
        const res: any = await toast.promise(create({ id: id, data: data, params }), {
          pending: 'در حال انجام...'
        })
        reset()
        onClose()
