...
import { useCreateMeetingAndDiagnosisInstitution1480 } from '@/hooks/institution/institution1480/useinstitution1480Institution'
import { GeTErrorFetch } from '@/components/elements/errorHandler'

export default function CreateMeet1480Form({ id, upsertData }: any) {
  const [userData, setUserData] = useState<any>(null)
  const fetchAdvisorInfo = async (userId: number) => {
    try {
      const res = await axiosConfig.get(`/institution/${id}/meeting-1480/core/meeting-1480/get-user`, {
        params: {
          user_id: userId
        }
      })
  ...
  const { mutateAsync, isPending }: any = useCreateMeetingAndDiagnosisInstitution1480()
                ...
                <>
                  ...
                  <Grid item xs={4}>
                    <Controller
                      ...
                      render={({ field: { onChange, value }, fieldState: { error } }) => (
                        <CustomAsyncAutocomplete
                          url={`/institution/${id}/meeting-1480/base/select/user`}
                          readOnly={false}
                          value={value}
